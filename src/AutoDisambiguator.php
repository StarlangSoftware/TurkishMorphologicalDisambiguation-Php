...
abstract class AutoDisambiguator
{
     ...
     * index.
     * @param int $index Position of the current word.
     * @param array $correctParses All correct morphological parses of the previous words.
     * @return bool True, if at least one of the morphological parses of the previous words has a singular second person
     * agreement or possessor tag, false otherwise.
     */
    private static function isAnyWordSecondPerson(int $index, array $correctParses): bool
    {
        $count = 0;
        for ($i = $index - 1; $i >= 0; $i--) {
     ...
     * @param int $index Position of the current word.
     * @param array $correctParses All correct morphological parses of the previous words.
     * @return bool True, if at least one of the morphological parses of the previous words has a plural agreement or
     * possessor tag, false otherwise.
     */
    private static function isPossessivePlural(int $index, array $correctParses): bool
    {
        for ($i = $index - 1; $i >= 0; $i--) {
     ...
     * @param FsmParseList $nextParseList All possible parses of the next word.
     * @return string Most frequent pos tag in all possible parses of the next word.
     */
    private static function nextWordPos(FsmParseList $nextParseList): string
    {
        $map = new CounterHashMap();
        for ($i = 0; $i < $nextParseList->size(); $i++) {
     ...
     * @param int $index Position of the current word.
     * @param array $fsmParses All morphological parses of the current sentence.
     * @return bool True, if the current word is just before the last word, false otherwise.
     */
    private static function isBeforeLastWord(int $index, array $fsmParses): bool
    {
        return $index + 2 == count($fsmParses);
    }
     ...
     * @param int $index Position of the current word.
     * @param array $fsmParses All morphological parses of the current sentence.
     * @return bool True, if there is at least one word after the current word, false otherwise.
     */
    private static function nextWordExists(int $index, array $fsmParses): bool
    {
        return $index + 1 < count($fsmParses);
    }
     ...
     * @param int $index Position of the current word.
     * @param array $fsmParses All morphological parses of the current sentence.
     * @return bool True, if there is at least one word after the current word and that next word is a noun, false otherwise.
     */
    private static function isNextWordNoun(int $index, array $fsmParses): bool
    {
        return $index + 1 < count($fsmParses) && self::nextWordPos($fsmParses[$index + 1]) == "NOUN";
    }
     ...
     * @param int $index Position of the current word.
     * @param array $fsmParses All morphological parses of the current sentence.
     * @return bool True, if there is at least one word after the current word and that next word is a number, false
     * otherwise.
     */
    private static function isNextWordNum(int $index, array $fsmParses): bool
    {
        return $index + 1 < count($fsmParses) && self::nextWordPos($fsmParses[$index + 1]) == "NUM";
    }
     ...
     * @param int $index Position of the current word.
     * @param array $fsmParses All morphological parses of the current sentence.
     * @return bool True, if there is at least one word after the current word and that next word is a noun or adjective,
     * false otherwise.
     */
    private static function isNextWordNounOrAdjective(int $index, array $fsmParses): bool
    {
        return $index + 1 < count($fsmParses) && (self::nextWordPos($fsmParses[$index + 1]) == "NOUN" || self::nextWordPos($fsmParses[$index + 1]) == "NOUN");
    }

    /**
     * Checks if the current word is the first word of the sentence or not.
     * @param int $index Position of the current word.
     * @return bool True, if the current word is the first word of the sentence, false otherwise.
     */
    private static function isFirstWord(int $index): bool
    {
        return $index === 0;
    }
     ...
     * @param array $fsmParses All morphological parses of the current sentence.
     * @param string $word 'ne', 'ya' or 'gerek'
     * @return bool True, if there are at least two occurrences of 'ne', 'ya' or 'gerek' in the sentence, false otherwise.
     */
    private static function containsTwoNeOrYa(array $fsmParses, string $word): bool
    {
        $count = 0;
        foreach ($fsmParses as $fsmPars) {
     ...
     * @param int $index Position of the current word.
     * @param array $correctParses All correct morphological parses of the previous words.
     * @param MorphologicalTag $tag Pos tag of the previous word
     * @return bool True, if there is at least one word before the given word and its pos tag is the given pos tag, false
     * otherwise.
     */
    private static function hasPreviousWordTag(int $index, array $correctParses, MorphologicalTag $tag): bool
    {
     ...
     * @param string $parseString Disambiguation parse string. The string contains distinct subparses for the given word for a
     *                    determined root word. The subparses are separated with '$'.
     * @param int $index Position of the current word.
     * @param array $fsmParses All morphological parses of the current sentence.
     * @param array $correctParses All correct morphological parses of the previous words.
     * @return string Correct morphological subparse of the current word.
     */
    private static function selectCaseForParseString(string $parseString, int $index, array $fsmParses, array $correctParses): string
    {
        $surfaceForm = $fsmParses[$index]->getFsmParse(0)->getSurfaceForm();
        $root = $fsmParses[$index]->getFsmParse(0)->getWord()->getName();
        $lastWord = $fsmParses[count($fsmParses) - 1]->getFsmParse(0)->getSurfaceForm();
        switch ($parseString) {
            /* kısmını, duracağını, grubunun */
            case "P2SG\$P3SG":
                if (self::isAnyWordSecondPerson($index, $correctParses)) {
                    return "P2SG";
                }
                return "P3SG";
            ...
            case "A3PL+P3PL+NOM\$A3PL+P3SG+NOM\$A3PL+PNON+ACC\$A3SG+P3PL+NOM":
                if (self::isPossessivePlural($index, $correctParses)) {
            ...
            case "ADJ\$ADV\$POSTP+PCABL":
                if (self::hasPreviousWordTag($index, $correctParses, MorphologicalTag::ABLATIVE)) {
                    return "POSTP+PCABL";
                }
                if ($index + 1 < count($fsmParses)) {
                    switch (self::nextWordPos($fsmParses[$index + 1])) {
                        case "NOUN":
                            return "ADJ";
                        case "ADJ":
                        case "ADV":
            ...
            case "ADJ\$NOUN+A3SG+PNON+NOM":
                if (self::isNextWordNounOrAdjective($index, $fsmParses)) {
                    return "ADJ";
                }
            ...
            case "DET\$PRON+DEMONSP+A3SG+PNON+NOM":
                if (self::isNextWordNoun($index, $fsmParses)) {
                    return "DET";
                }
                return "PRON+DEMONSP+A3SG+PNON+NOM";
            /* gelebilir */
            case "AOR+A3SG\$AOR^DB+ADJ+ZERO":
                if (self::isBeforeLastWord($index, $fsmParses)) {
                    return "AOR+A3SG";
                } else if (self::isFirstWord($index)) {
            ...
            case "ADJ^DB+VERB+ZERO\$VERB+POS":
                if ($root == "var" && !self::isPossessivePlural($index, $correctParses)) {
            ...
            case "ADJ\$ADV\$NOUN+A3SG+PNON+NOM":
                if ($root == "artık") {
                    return "ADV";
                } else if (self::isNextWordNoun($index, $fsmParses)) {
                    return "ADJ";
                }
                return "ADV";
            case "P1SG+NOM\$PNON+NOM^DB+VERB+ZERO+PRES+A1SG":
                if (self::isBeforeLastWord($index, $fsmParses) || $root == "değil") {
            ...
            case "ADJ\$ADV\$CONJ\$PRON+QUESP+A3SG+PNON+NOM":
                ...
                if (self::containsTwoNeOrYa($fsmParses, "ne")) {
                    return "CONJ";
                }
                if (self::isNextWordNoun($index, $fsmParses)) {
                    return "ADJ";
            ...
            case "ADJ\$ADV\$NOUN+A3SG+PNON+NOM\$POSTP+PCDAT":
                if (self::hasPreviousWordTag($index, $correctParses, MorphologicalTag::DATIVE)) {
                    return "POSTP+PCDAT";
                }
                if (self::isNextWordNoun($index, $fsmParses)) {
                    return "ADJ";
            ...
            /* YA */
            case "CONJ\$INTERJ":
                if (self::containsTwoNeOrYa($fsmParses, "ya")) {
                    return "CONJ";
                }
                if (self::nextWordExists($index, $fsmParses) && $fsmParses[$index + 1]->getFsmParse(0)->getSurfaceForm() == "da") {
                    return "CONJ";
                }
                return "INTERJ";
            ...
            case "ADV\$POSTP+PCINS":
                if (self::hasPreviousWordTag($index, $correctParses, MorphologicalTag::INSTRUMENTAL)) {
                    return "POSTP+PCINS";
                }
                return "ADV";
            ...
            case "NOUN+A3SG+PNON+NOM\$NUM+CARD\$VERB+POS+IMP+A2SG":
                if (self::isNextWordNum($index, $fsmParses)){
                    return "NUM+CARD";
                }
                return "NOUN+A3SG+PNON+NOM";
            ...
            case "CONJ\$NOUN+A3SG+PNON+NOM\$VERB+POS+IMP+A2SG":
                if (self::containsTwoNeOrYa($fsmParses, "gerek")){
                    return "CONJ";
                }
                return "NOUN+A3SG+PNON+NOM";
     ...
     * @param array $fsmParses All morphological parses of the current sentence.
     * @param array $correctParses All correct morphological parses of the previous words.
     * @return FsmParse|null Correct morphological parse of the current word.
     */
    public static function caseDisambiguator(int $index, array $fsmParses, array $correctParses): ?FsmParse
    {
        $fsmParseList = $fsmParses[$index];
        $defaultCase = self::selectCaseForParseString($fsmParses[$index]->parsesWithoutPrefixAndSuffix(), $index, $fsmParses, $correctParses);
        if ($defaultCase != "") {
